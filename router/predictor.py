...
class Predictor():
    "This class will realize the predictions."
    ...
    def predict_all_values(self, features_set: pd.DataFrame):
        "This method creates a prediction given real data."

        numeric_features = features_set[self._NUMERIC_FEATURES].values
        ...
        result = {
            ...
            't_hour': int(features_set['hour'].values[0]),
            't_day': int(features_set['day'].values[0]),
            't_month': int(features_set['month'].values[0]),
            't_year': int(features_set['year'].values[0])
        }
